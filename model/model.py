...
class ConnectivityEM:
    ...
    def negative_log_likelihood(self, X):
        ...
        for i in range(self.N):
            ...
            log_likelihood += 0.5*(cst + det_sigma + mahalanobis)
        return log_likelihood.sum()/np.prod(X.shape[:2])
    def fit(self, lr = 1e-3, reg = 1e-2, tol=1e-3):
        ...
        while np.abs(new_likelihood - old_likelihood) > tol:
            ...
            W_grad = np.zeros(self.W.shape)
            # E-step
            self.mu_z = np.array([[(self.W @ self.G[i]).T @ \
                             np.linalg.lstsq(self.sigmas[i], self.X[i][j], rcond=None)[0] for j in range(self.n)] for i in range(self.N)])
            self.sigmas_z = np.array([self.G[i] - (self.W @ self.G[i]).T @ \
                                  np.linalg.lstsq(self.sigmas[i], self.W @ self.G[i], rcond=None)[0] for i in range(self.N)])
            self.E_z = self.mu_z
            ...
            for i in range(self.N):
                for j in range(self.n):
                    ...
                    self.v[i] += (0.5*np.sum(self.X[i][j]**2) - self.E_z[i][j] @ self.W.T @ self.X[i][j]) +\
                                  0.5*np.trace(self.E_z_2[i][j] @ self.W.T @ self.W)
                self.G[i] /= self.n
